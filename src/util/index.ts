...
export const stringGenerator = (len: number, mix: boolean): string => {
  if (mix)
    return [...Array(len)]
      .map(() =>
        wordGenerator(Math.floor(Math.random() * 7) + 2, Math.random() >= 0.8)
      )
      .join(" ");
...
export const randomUser = () => randomArrayElement(tempUsers);
...
export const previousRandomDate = () => {
  return new Date(
    1000000000000 + Math.random() * (new Date().getTime() - 1000000000000)
  );
};



export const uniqeString=()=> {
  return "xxxxxxxx-yxxxxxx".replace(/[xy]/g, (c)=> {
